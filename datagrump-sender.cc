...
int main( int argc, char *argv[] )
{
  ...
  if ( argc == 4 && string( argv[ 3 ] ) == "debug" ) {
    ...
    /* do nothing */
		...
		} else if ( string( argv[ 4 ]) == "aimd" && argc == 7){
			ai_coeff = strtod(argv[5], NULL);
			md_coeff = strtod(argv[6], NULL);
			controllerType = AIMD;
		} else {
  ...
  try {
    ...
    /* Initialize flow controller */
   	Controller controller( debug, init_cwnd, controllerType );
		if(controllerType == AIMD) {
			controller.setup_aimd(ai_coeff, md_coeff);
		}
